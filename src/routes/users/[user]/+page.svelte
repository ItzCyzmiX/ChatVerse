...
<div class="min-h-screen text-white p-8">
    ...
    <div class="max-w-7xl mx-auto">
        <div class="mb-12 bg-zinc-800/30 rounded-xl p-8 border-2 border-white/20">
            <div class="flex items-center gap-8">
                ...
                />
                <div class="flex-grow">
                    <h1 class="text-4xl font-bold mb-2">{userData.name}</h1>
                    <p class="text-zinc-400">@{userData.name}</p>
                </div>
                <button 
                    class="bg-red-500/20 text-red-500 px-6 py-2 rounded-lg hover:bg-red-500/30 transition-colors"
                    onclick={logout}
                >
                    Log Out
                </button>
        ...
        <section class="mb-12">
            <h2 class="text-3xl font-bold mb-6">Created Chatbots</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {:else}
                    ...
                    <div class="bg-zinc-800/30 rounded-xl p-6 transition-all duration-200 hover:transform hover:-translate-y-1 hover:bg-zinc-900 cursor-pointer border-2 border-white/20">
                        <h3 class="text-xl font-bold mb-2">{bot.name}</h3>
                        <p class="text-zinc-400 text-sm">{bot.title}</p>

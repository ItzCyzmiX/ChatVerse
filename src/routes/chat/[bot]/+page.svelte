...
<div> 
    <button 
        ...
        class="fixed top-20 right-4 bg-zinc-800/30 border-2 border-white/20 rounded-xl p-2 text-white hover:bg-zinc-900/30 transition-all duration-200 flex items-center gap-2" 
    ...
    {#if showDetails}
        <div class="fixed inset-0 bg-transparent z-50 flex items-center justify-center" transition:scale>
    ...
    {/if}
    <div class="h-screen flex flex-col {showDetails ? 'blur-sm' : ''}">
        ...
        <!-- Header -->
        <div class="bg-zinc-900/30 border-b-2 border-white/20 p-4 backdrop-blur-xl">
            <div class="max-w-7xl mx-auto flex justify-center items-center relative">
                <div class="text-center">
                    <h1 class="text-2xl font-bold text-white">{botName}</h1>
                    <p class="text-sm text-zinc-400">Created by {botCreator}</p>
                </div>
                <a aria-label="Back to chats" href="/chats" class="absolute right-0 text-red-600 ">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </a>
        ...
        <div class="bg-transparent p-4">
            <div class="max-w-3xl mx-auto">
                <form onsubmit={handleSubmit} class="flex gap-4">
                    ...
                    <button
                        type="submit"
                        class="bg-white font-bold text-black px-6 py-2 rounded-xl font-medium transition-colors"
                    >
                        Send
                    </button>
                </form>
